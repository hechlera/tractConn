...
while IFS=',' read SUBJ_ID; do
	...
	if [ ! -d ${ANAT_PATH}/dtifit ]; then
		echo "Creating output folder for dtifit"
		mkdir ${ANAT_PATH}/dtifit
	else
	...
	# run dtifit
	dtifit -k ${DWI_BCOR} -o ${DTIFIT_PATH}/dtifit -m ${DWI_MASK_D} -r ${BVEC_FILE} -b ${BVAL_FILE}
	...
	antsBrainExtraction -d 3 -a ${MPRAGE_FOV} -e ${MNI2mm_whole} -m ${MNI2mm_mask} -o ${ANTS_FOLDER}/
