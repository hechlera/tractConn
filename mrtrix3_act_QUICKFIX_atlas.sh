...
######################## TO DO ########################
### - ATLAS option:
### 	- lookup table for different atlas?
###			- expectation: Integer values starting at 1
###		- output name hardcoded to "Yeo400"
...
while getopts "s:i:o:a:h" opt; do
...
while IFS=',' read SUBJ_ID; do
	...
	if [[ -z ${ATLAS} || -z ${MNI2mm} || -z ${MNI2mm_mask} || -z ${MNI2mm_whole} ]]; then
		echo 'ERROR: Atlas and/or MNI2mm/MNI2mm mask template were not found.'
		FLAG=1
	fi
